...
const mockAlertesBase: Alerte[] = [
  {
    ...
    description:
      "Le mandat de Marie Dubois en tant que Président du praesidium Notre-Dame du Rosaire se termine dans 15 jours.",
    priorite: "haute",
  ...
  },
  {
    ...
    description:
      "Le praesidium Saint-Jean-Baptiste n'a pas tenu sa réunion hebdomadaire depuis 2 semaines.",
    priorite: "moyenne",
    date_creation: new Date("2024-01-25"),
    statut: "active",
    entite_concernee: "Saint-Jean-Baptiste",
    actions_suggerees: [
      "Contacter le pr��sident",
  ...
  },
  {
    ...
    description:
      "Le praesidium Sainte-Thérèse n'a pas soumis son rapport financier du mois dernier.",
    priorite: "moyenne",
  ...
  },
  {
    ...
    description:
      "Pierre Laurent n'a pas participé aux réunions depuis 3 mois. Suivi requis.",
    priorite: "basse",
  ...
  },
  {
    ...
    description:
      "Jean Dupont est probationnaire depuis 18 mois. Il peut maintenant faire sa promesse.",
    priorite: "moyenne",
  ...
  },
  {
    ...
    description:
      "Le mandat de Sophie Martin comme secrétaire s'est terminé et un nouveau secrétaire a été élu.",
    priorite: "basse",
...
export default function Alerts() {
  ...
  const checkProbationAlerts = () => {
    ...
    const nouvellesAlertesProbation: AlerteProbation[] =
      membresEnProbationLongue.map((membre) => {
        const praesidium = mockPraesidia.find(
          (p) => p.id_praesidium === membre.id_praesidium,
        );
        const dureeMois = Math.floor(
          (now.getTime() - membre.date_adhesion.getTime()) /
            (30 * 24 * 60 * 60 * 1000),
        );
    ...
    const alertesProbationConverties: Alerte[] = nouvellesAlertesProbation.map(
      (alert) => ({
        ...
        date_echeance: new Date(
          alert.date_debut_probation.getTime() + 6 * 30 * 24 * 60 * 60 * 1000,
        ), // 6 mois max
    ...
    setAlertes((prev) => {
      ...
      const alertesSansProbation = prev.filter(
        (a) => a.type !== "probation_prolongee",
      );
  ...
  const filteredAlertes = useMemo(() => {
    return alertes.filter((alerte) => {
      ...
      const matchesType =
        selectedType === "all" || alerte.type === selectedType;
      const matchesPriorite =
        selectedPriorite === "all" || alerte.priorite === selectedPriorite;
      const matchesStatut =
        selectedStatut === "all" || alerte.statut === selectedStatut;
  ...
  const stats = {
    ...
    hautes: alertes.filter(
      (a) => a.priorite === "haute" && a.statut === "active",
    ).length,
    probations: alertes.filter(
      (a) => a.type === "probation_prolongee" && a.statut === "active",
    ).length,
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      ...
      case "reunion_manquee":
        return <Calendar className="h-4 w-4 text-red-500" />;
      case "contribution_manquante":
  ...
  const getTypeName = (type: string) => {
    switch (type) {
      case "fin_mandat":
        return "Fin de mandat";
      case "reunion_manquee":
        return "Réunion manquée";
      case "contribution_manquante":
        return "Contribution manquante";
      case "membre_inactif":
        return "Membre inactif";
      case "promesse_due":
        return "Promesse due";
      case "probation_prolongee":
  ...
  const handleStatusChange = (
    alerteId: string,
    newStatus: "active" | "resolue" | "ignoree",
  ) => {
    setAlertes(
      alertes.map((alerte) =>
  ...
  const handleProbationAlert = (
    alerteId: string,
    action: "promesse" | "formation" | "ignore",
  ) => {
    const alerteProbation = alertesProbation.find(
      (a) => a.id_alerte === alerteId,
    );
    ...
    <div className="space-y-6">
      ...
      {stats.probations > 0 && (
        <Alert>
          ...
          <AlertDescription>
            <strong>Nouveautés :</strong> Le système détecte automatiquement les
            membres en probation depuis plus de 3 mois et alerte les officiers
            pour organiser leur promesse. {stats.probations} membre(s)
            concerné(s).
          </AlertDescription>
        </Alert>
      )}
          ...
          <div className="flex items-center gap-4">
            ...
            <Select value={selectedType} onValueChange={setSelectedType}>
              ...
              <SelectContent>
                ...
                <SelectItem value="contribution_manquante">
            ...
            <Select
              ...
              onValueChange={setSelectedPriorite}
            >
        ...
        {filteredAlertes.map((alerte) => {
          ...
          return (
            <Card
              ...
              className={`${
                alerte.priorite === "haute" && alerte.statut === "active"
              ...
              <CardContent className="p-4">
                <div className="flex items-start justify-between">
                  <div className="flex items-start gap-3 flex-1">
                    <div className="mt-0.5">{getTypeIcon(alerte.type)}</div>
                  ...
                  <div className="flex items-center gap-2 ml-4">
                        ...
                        {alerte.type === "probation_prolongee" ? (
                          <>
                            ...
                            <Button
                              ...
                              variant="ghost"
                              onClick={() =>
                                handleProbationAlert(alerte.id_alerte, "ignore")
                              }
                              className="text-xs"
                            >
                              Reporter
                          ...
                          <>
                            <Button
                              ...
                              variant="outline"
                              onClick={() =>
                                handleStatusChange(alerte.id_alerte, "resolue")
                              }
                              className="text-xs"
                            >
                            ...
                            <Button
                              ...
                              variant="ghost"
                              onClick={() =>
                                handleStatusChange(alerte.id_alerte, "ignoree")
                              }
                              className="text-xs"
                            >
                              Ignorer
                    ...
                    {alerte.statut !== "active" && (
                      <Button
                        ...
                        variant="ghost"
                        onClick={() =>
                          handleStatusChange(alerte.id_alerte, "active")
                        }
                        className="text-xs"
                      >
                        Réactiver
