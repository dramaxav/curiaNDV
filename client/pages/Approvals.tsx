...
import type { DemandeCompte, ApprobationPresence, ApprobationFinance, Praesidium } from '@shared/types';
...
const mockDemandesCompte: DemandeCompte[] = [
  {
    ...
    justification: 'Je suis membre active de Notre-Dame du Rosaire depuis 3 ans et souhaite servir davantage la communauté en tant que secrétaire.',
    statut: 'en_attente',
  ...
  },
  {
    ...
    justification: 'Fort de 10 ans d\'expérience dans la Légion de Marie et d\'une formation en théologie, je souhaite contribuer à la formation des membres.',
    statut: 'en_attente',
...
export default function Approvals() {
  ...
  const [demandesCompte, setDemandesCompte] = useState<DemandeCompte[]>(mockDemandesCompte);
  const [approbationsPresence, setApprobationsPresence] = useState<ApprobationPresence[]>(mockApprobationsPresence);
  const [approbationsFinance, setApprobationsFinance] = useState<ApprobationFinance[]>(mockApprobationsFinance);
  ...
  const getStatutBadge = (statut: string) => {
    switch (statut) {
      ...
      case 'approuvee': 
        return <Badge variant="default"><CheckCircle className="h-3 w-3 mr-1" />Approuvée</Badge>;
      case 'refusee': 
  ...
  const stats = {
    comptes_en_attente: demandesCompte.filter(d => d.statut === 'en_attente').length,
    presences_en_attente: approbationsPresence.filter(a => a.statut === 'en_attente').length,
    finances_en_attente: approbationsFinance.filter(a => a.statut === 'en_attente').length,
    total_en_attente: demandesCompte.filter(d => d.statut === 'en_attente').length + 
                     approbationsPresence.filter(a => a.statut === 'en_attente').length +
                     approbationsFinance.filter(a => a.statut === 'en_attente').length
  };

  const handleApproval = (type: 'compte' | 'presence' | 'finance', id: string, action: 'approve' | 'reject') => {
  ...
  const confirmApproval = () => {
    ...
    if (type === 'compte') {
      setDemandesCompte(prev => prev.map(demande => 
      ...
      setApprobationsPresence(prev => prev.map(approbation => 
      ...
      setApprobationsFinance(prev => prev.map(approbation => 
        approbation.id_approbation === id 
          ? {
              ...approbation,
              statut: newStatut as any,
  ...
  const filteredDemandesCompte = useMemo(() => {
    return demandesCompte.filter(demande => 
      ...
      <div className="container mx-auto py-8 px-4">
        <Card className="max-w-md mx-auto">
          <CardHeader className="text-center">
            ...
            <CardDescription>
              Vous n'avez pas les permissions nécessaires pour accéder aux approbations
            </CardDescription>
          </CardHeader>
        </Card>
      </div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Système d'Approbations</h1>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        {canApproveAccounts && (
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{stats.comptes_en_attente}</div>
            </CardContent>
          </Card>
        )}
        {canApprovePresences && (
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{stats.presences_en_attente}</div>
            </CardContent>
          </Card>
        )}
        {canApproveFinances && (
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{stats.finances_en_attente}</div>
            </CardContent>
          </Card>
        )}
      </div>
                    ...
                    <TableBody>
                      {filteredDemandesCompte.map((demande) => (
                        <TableRow key={demande.id_demande}>
                          <TableCell>
                            <div>
                              <div className="font-medium">{demande.nom_prenom}</div>
                          ...
                          <TableCell>
                            <div>
                              <div className="font-medium">{demande.poste_souhaite}</div>
                              <div className="text-sm text-muted-foreground">
                                {demande.type_demande === 'officier_conseil' ? 'Officier du Conseil' : 'Officier de Praesidium'}
                              </div>
                            </div>
                          </TableCell>
                          <TableCell>
                            ...
                            {demande.statut === 'en_attente' && (
                              <div className="flex items-center justify-end gap-2">
                                ...
                                <Button
                                  variant="outline"
                                  size="sm"
                                  onClick={() => handleApproval('compte', demande.id_demande, 'approve')}
                                ...
                                <Button
                                  variant="outline"
                                  size="sm"
                                  onClick={() => handleApproval('compte', demande.id_demande, 'reject')}
                    ...
                    <TableBody>
                      {approbationsPresence.map((approbation) => (
                        <TableRow key={approbation.id_approbation}>
                          ...
                          <TableCell>
                            {approbation.date_soumission.toLocaleDateString('fr-FR')}
                          </TableCell>
                          <TableCell>
                            ...
                            {approbation.statut === 'en_attente' && (
                              <div className="flex items-center justify-end gap-2">
                                <Button
                                  variant="outline"
                                  size="sm"
                                  onClick={() => handleApproval('presence', approbation.id_approbation, 'approve')}
                                  className="text-green-600 hover:text-green-700"
                                >
                                  <CheckCircle className="h-4 w-4 mr-1" />
                                  Approuver
                                ...
                                <Button
                                  variant="outline"
                                  size="sm"
                                  onClick={() => handleApproval('presence', approbation.id_approbation, 'reject')}
                                  className="text-red-600 hover:text-red-700"
                                >
                                  <XCircle className="h-4 w-4 mr-1" />
                                  Refuser
                            ...
                            {approbation.statut === 'en_attente' && (
                              <div className="flex items-center justify-end gap-2">
                                <Button
                                  variant="outline"
                                  size="sm"
                                  onClick={() => handleApproval('finance', approbation.id_approbation, 'approve')}
                                  className="text-green-600 hover:text-green-700"
                                >
                                  <CheckCircle className="h-4 w-4 mr-1" />
                                  Approuver
                                ...
                                <Button
                                  variant="outline"
                                  size="sm"
                                  onClick={() => handleApproval('finance', approbation.id_approbation, 'reject')}
                                  className="text-red-600 hover:text-red-700"
                                >
                                  <XCircle className="h-4 w-4 mr-1" />
                                  Refuser
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {currentAction?.action === 'approve' ? 'Approuver' : 'Refuser'} la demande
            </DialogTitle>
            <DialogDescription>
          ...
          <div className="space-y-4">
            <div>
              <Label htmlFor="commentaire">
                {currentAction?.action === 'approve' ? 'Commentaire (optionnel)' : 'Raison du refus *'}
              </Label>
              <Textarea
                id="commentaire"
                value={commentaire}
          ...
          <div className="flex justify-end space-x-2">
            ...
            <Button 
              ...
              variant={currentAction?.action === 'approve' ? 'default' : 'destructive'}
